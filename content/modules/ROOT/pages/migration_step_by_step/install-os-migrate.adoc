...
In the *vms_list* we specify the list of VMs to be migrated in this example, we will migrate the virtual machine haproxy-{fvmw_vcenter_user}
...
Power on the virtual machine haproxy-{fvmw_vcenter_user}
...
[source,bash,role=execute,subs=attributes]
----
openstack server add floating ip haproxy-{fvmw_vcenter_user} $FLOATING_IP_HA_PROXY
----
...
+--------------------------------------+---------------+--------+-----------------------------------------+--------------------------+--------------------------+
| 77844fc9-1c83-4328-8995-3c9a008e8134 | haproxy-{fvmw_vcenter_user} | ACTIVE | private=10.0.0.25, FLOATING_IP | N/A (booted from volume) | osm-vmware-haproxy-tztc2 |
...
[source,bash,role=execute,subs=attributes]
----
openstack server delete haproxy-{fvmw_vcenter_user}
----