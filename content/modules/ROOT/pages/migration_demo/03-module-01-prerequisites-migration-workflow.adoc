...
* **vcenter_hostname**: fvmw server address (`{fvmw_hostname}`)
* **vcenter_username/password**: Credentials for accessing fvmw
* **local_disk_path**: Path to pre-staged VMDK disks on the conversion host (`/local/disks`)
...
6. Verify the `vms_list` variable contains: `haproxy-{fvmw_vcenter_user}`
